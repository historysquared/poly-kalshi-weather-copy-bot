...
class CliDailyRecord:
    ...
    nws_product_id: str | None = None
    nws_product_link: str | None = None
    wfo: str | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        row = asdict(self); row["valid_date"] = self.valid_date.isoformat(); return row


def _num(value: Any, *, trace: float | None = 0.0) -> float | None:
    if value is None: return None
    text = str(value).strip()
    if not text or text.upper() in {"M", "MM", "NULL", "NONE", "NAN"}: return None
    if text.upper() == "T": return trace
...
def parse_iem_cli_payload(station: str, payload: Any) -> list[CliDailyRecord]:
    """Parse IEM's archive of atomic values from NWS-issued CLI products.

    IEM is the archive/index layer; the NWS CLI product identity is retained so
    each row can be audited back to the underlying official text product.
    """
    if isinstance(payload, dict): rows = payload.get("results") or payload.get("data") or payload.get("records") or payload.get("cli") or []
    ...
    for row in rows:
        ...
        product = _clean(_first(row, "product"))
        link = _clean(_first(row, "link"))
        out.append(CliDailyRecord(
            station=station.upper(), valid_date=valid,
            ...
            nws_product_id=product,
            nws_product_link=("https://mesonet.agron.iastate.edu" + link) if link and link.startswith("/") else link,
            wfo=_clean(_first(row, "wfo")), station_name=_clean(_first(row, "name")), raw=dict(row),
        ))
    return sorted(out, key=lambda x: x.valid_date)


class IemCliArchive:
    def __init__(self, timeout_s: float = 30.0) -> None: self.timeout_s = timeout_s
    ...
    def fetch_year(self, station: str, year: int) -> list[CliDailyRecord]:
        with httpx.Client(timeout=self.timeout_s, follow_redirects=True, headers={"User-Agent": "weather-alpha-lab/0.3"}) as client:
            response = client.get(IEM_CLI_URL, params={"station": station.upper(), "year": int(year), "fmt": "json"})
    ...
    def fetch_years(self, station: str, years: Iterable[int]) -> list[CliDailyRecord]:
        ...
        for year in sorted(set(int(y) for y in years)):
            for row in self.fetch_year(station, year): by_date[row.valid_date] = row
