...
def _text(meta: dict[str, Any]) -> str:
    return " ".join(str(meta.get(key) or "") for key in
                    ("ticker", "event_ticker", "title", "subtitle", "yes_sub_title", "rules_primary", "rules_secondary"))
...
def looks_weather_like(meta: dict[str, Any]) -> bool:
    ...
    return bool(series in _KNOWN_HIGH_SERIES or series in _KNOWN_LOW_SERIES or _TEMP_TERMS.search(text) or _PRECIP_TERMS.search(text))


def infer_measurement(meta: dict[str, Any]) -> WeatherMeasure:
    text = _text(meta).lower()
    series = _series_ticker(meta)
    if series in _KNOWN_HIGH_SERIES or "daily high" in text or "high temperature" in text:
        return WeatherMeasure.DAILY_HIGH
    if series in _KNOWN_LOW_SERIES or "daily low" in text or "low temperature" in text:
...
def infer_shape(meta: dict[str, Any]) -> tuple[ContractShape | None, float | None, float | None]:
    floor = _num(meta.get("floor_strike")); cap = _num(meta.get("cap_strike"))
    if floor is not None and cap is not None: return ContractShape.BUCKET, floor, cap
    if floor is not None: return ContractShape.ABOVE, floor, None
    if cap is not None: return ContractShape.BELOW, None, cap
    return None, None, None


def classify_kalshi_market(meta: dict[str, Any]) -> WeatherCatalogRecord:
    ticker = str(meta.get("ticker") or ""); event_ticker = str(meta.get("event_ticker") or "")
    title = str(meta.get("title") or ""); subtitle = str(meta.get("yes_sub_title") or meta.get("subtitle") or "")
    reasons: list[str] = []
    if not ticker:
        return WeatherCatalogRecord("kalshi", "", event_ticker, CatalogStatus.REJECT, WeatherMeasure.OTHER, None, None, None, None, None, None, title, subtitle, None, ("missing ticker",), meta)
    if not looks_weather_like(meta):
        return WeatherCatalogRecord("kalshi", ticker, event_ticker, CatalogStatus.REJECT, WeatherMeasure.OTHER, None, None, None, None, None, None, title, subtitle, None, ("no explicit meteorological evidence",), meta)
    measure = infer_measurement(meta)
    if measure == WeatherMeasure.OTHER:
        return WeatherCatalogRecord("kalshi", ticker, event_ticker, CatalogStatus.REJECT, measure, None, None, None, None, None, None, title, subtitle, None, ("weather-like candidate has no supported measurement",), meta)
    station = explicit_station(meta); settle_date = explicit_settlement_date(meta); shape, lower, upper = infer_shape(meta)
    source_text = " ".join(str(meta.get(k) or "") for k in ("rules_primary", "rules_secondary")); source = source_text.strip() or None
    ...
    if shape is None and measure in {WeatherMeasure.DAILY_HIGH, WeatherMeasure.DAILY_LOW, WeatherMeasure.TEMPERATURE}: reasons.append("temperature contract bounds unavailable")
    ...
    if measure in {WeatherMeasure.DAILY_HIGH, WeatherMeasure.DAILY_LOW, WeatherMeasure.TEMPERATURE}: exact = exact and shape is not None
    status = CatalogStatus.EXACT if exact else (CatalogStatus.PROBABLE if station is not None or source_verified else CatalogStatus.REVIEW)
    ...
    elif not isinstance(close, datetime): close = None
    return WeatherCatalogRecord("kalshi", ticker, event_ticker, status, measure, station, settle_date, shape, lower, upper, source, title, subtitle, close, tuple(reasons), meta)
